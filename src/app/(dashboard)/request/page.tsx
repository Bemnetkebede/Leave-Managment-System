...
=======
import { useLeaveStats } from '@/hooks/queries/leaveQueries';
...
=======
  ...
  const isOverBalance = workingDays > availableBalance;
  ...
  useEffect(() => {
    const fetchDays = async () => {
      if (fromDate) {
        try {
          const start = fromDate;
          const end = isMultipleDays ? (toDate || fromDate) : fromDate;
          const days = await getWorkingDaysFromDB(supabase, start, end);
          setWorkingDays(days);
        } catch (err) {
    ...
    fetchDays();
  }, [fromDate, toDate, isMultipleDays, supabase]);
>>>>>>> 216fc4495fa2672be9db4277c8591826e7bdd72b
...
=======
    ...
    try {
        ...
        const payload: any = {
            ...
            leave_type: category,
            start_date: fromDate,
            end_date: isMultipleDays ? toDate : fromDate,
            reason: reason,
            status: 'pending'
        };
